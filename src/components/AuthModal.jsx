...
function AuthModal({ isOpen, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isLogin) {
            ...
            try {
                const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/v1/auth/authenticate`, {
                    method: 'POST',
                ...
                if (data.access_token) {
                    ...
                    login({ name: data.username || name || email.split('@')[0], email });
                    alert('¡Login exitoso!');
                    onClose();
                    // Limpiar campos
                    setEmail('');
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    try {
                        ...
                        if (res.status === 409 || errorMessage.toLowerCase().includes('duplicate') || 
                            ...
                            // Probablemente un DataIntegrityViolationException
                ...
                if (data.access_token) {
                    localStorage.setItem('token', data.access_token);
                    localStorage.setItem('authToken', data.access_token);
                    register({ name: `${name} ${lastname}`, email });
